...
async def auto_state(_, message):
    ...
    if user.status in (
        ...
        state = message.text.split(None, 1)[1].strip().lower() if len(message.command) > 1 else "enable"
...
async def greet_group(_, member: ChatMemberUpdated):
    ...
    try:
        ...
        os.remove(welcomeimg)  # Remove the generated welcome image after sending
...
async def bot_wel(_, message):
    for u in message.new_chat_members:
        if u.id == app.me.id:
            ...
            try:
                pic = await app.get_chat(u.id).photo.big_file_id
                welcomeimg = welcomepic(
                    pic, app.get_me().first_name, message.chat.title, app.get_me().id, app.get_me().username
                )
                await app.send_photo(
                    message.chat.id,
                    photo=welcomeimg,
                    caption=f"""
                        ───────────────────
                        Nᴀᴍᴇ: {app.get_me().mention}
                        Iᴅ: {app.get_me().id}
                        UsᴇʀNᴀᴍᴇ: @{app.get_me().username}
                        ───────────────────**
                    """
                )
                os.remove(welcomeimg)  # Remove the generated welcome image after sending
